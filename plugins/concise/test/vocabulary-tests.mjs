...
eq("one tier-2 word alone passes", scanAiWriting("The harness runs the suite.", { categories: ["vocabulary"] }), []);
eq(
...
eq("allow drops a matching hit", scanAiWriting("A robust parser.", { categories: ["vocabulary"], allow: ["robust"] }), []);
eq("allow is case-insensitive", scanAiWriting("A Robust parser.", { categories: ["vocabulary"], allow: ["ROBUST"] }), []);
eq(
...
eq(
  ...
  scanAiWriting("A robust parser that we delve into.", { categories: ["vocabulary"], allow: ["robust"] }).map((h) => h.match),
  ["delve"],
);
